...
#naics basis
exports_naics<-getCensus(
  name = "timeseries/intltrade/exports/naics", # this says where to look there are dozens of options
  vars = c("ALL_VAL_MO"), #this is the variable you want: here General exports by Month
  time = "from 2016",
...
# note there is also a cumulative export and expor value with suffix YR
exports_naics<-getCensus(
  name = "timeseries/intltrade/exports/naics",
  vars = c("ALL_VAL_YR","ALL_VAL_MO","YEAR"),
  time = "from 2023",
  CTY_CODE="1220",
  CTY_CODE="2010"
)
...
#ALL_VAL_YR is cumulative exports for consumption by month
#the end of the year value for this is annual total, month=12
exports_naics<-getCensus(
  name = "timeseries/intltrade/exports/naics",
  vars = c("ALL_VAL_YR","YEAR"),
  time = "from 2016",
  CTY_CODE="1220",
  CTY_CODE="2010",
...
## I want a make a graph of the top 10 export partners for any given year
...
#also, the values for year and exports are not numeric
...
#set the year
yrplot <- c(2015, 2025)
export_graph <- ggplot(top10_data %>% filter(YEAR %in% yrplot), aes(group = CTY_NAME, y = rank)) +
  ...
  facet_wrap(~ YEAR) +
  labs(x = "Export Value (billions USD)", y = "Ranking by Exports", title = paste("Top 10 Countries for U.S. Exports: 2015 and 2025")) +
  theme_minimal()
...
p<-ggplot(top10_data,aes(group = CTY_NAME, y = rank)) +
  ...
  scale_y_reverse(breaks = 1:10, minor_breaks = NULL)+
  facet_wrap(~ YEAR) +
  labs(x = "Export Value (billions USD)", y = "Ranking by Exports", title = 'Top 10 Source Countries for U.S. Exports: Year {closest_state}') +
  theme_minimal()
